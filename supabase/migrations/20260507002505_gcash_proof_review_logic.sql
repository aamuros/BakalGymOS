alter table public.gcash_proofs
  ...
  alter column proof_status set default 'pending_proof';

create index if not exists gcash_proofs_created_at_idx
on public.gcash_proofs(created_at);

create index if not exists gcash_proofs_review_queue_idx
on public.gcash_proofs(proof_status, created_at)
where proof_status in ('staff_checked', 'disputed', 'needs_follow_up');
...
where id = 'gcash-proofs';

update public.payments p
set status = case gp.proof_status
  when 'pending_proof' then 'pending_proof'::public.payment_status
  when 'pending_review' then 'staff_checked'::public.payment_status
  when 'staff_checked' then 'staff_checked'::public.payment_status
  when 'owner_confirmed' then 'owner_confirmed'::public.payment_status
  when 'disputed' then 'disputed'::public.payment_status
  when 'needs_follow_up' then 'needs_follow_up'::public.payment_status
  when 'verified' then 'owner_confirmed'::public.payment_status
  when 'rejected' then 'disputed'::public.payment_status
  else p.status
end
...
using (
  ...
  and proof_status in ('pending_proof', 'needs_follow_up', 'disputed')
)
with check (
  private.is_front_desk_or_management()
  and uploaded_by = auth.uid()
  and proof_status = 'staff_checked'
);
...
using (
  ...
  and status in ('pending_proof', 'disputed', 'needs_follow_up')
)
with check (
  private.is_front_desk_or_management()
  and received_by = auth.uid()
  and payment_type = 'gcash'
  and status = 'staff_checked'
);

drop policy if exists "gcash proofs storage read allowed staff" on storage.objects;
